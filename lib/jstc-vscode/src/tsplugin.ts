...
export class TsPlugin {
    ...
    static isEnabled(): boolean {
        return workspace.getConfiguration('jstc').get<boolean>('enable-ts-plugin') ?? false;
    }
    ...
    private async toggleTsPlugin(enable: boolean) {
        ...
        commands.executeCommand('_typescript.configurePlugin', 'typescript-jstc-plugin', {
            enable
        });
    }
    ...
    async askToEnable() {
        ...
        if (response === answers[2]) {
            ...
            workspace.getConfiguration('jstc').update('ask-to-enable-ts-plugin', false, true);
        }
    }
}
